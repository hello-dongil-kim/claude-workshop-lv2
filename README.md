...
> **팁:** 이 체크리스트를 복사해서 프로젝트 폴더에 `progress.md`로 저장해 두면, 다음에 이어서 진행할 때 편합니다. 각 세션을 다시 시작하면 튜터가 `progress.md`를 읽고 완료한 블록을 확인한 뒤 "이어서 진행할까요?"라고 제안하며, 블록을 마칠 때마다 해당 줄을 자동으로 `[x]`로 갱신합니다. 이미 아는 블록은 "이 블록 알아요"라고 말하면 1분 요약 후 퀴즈로 건너뜁니다.
