...
**사례 1: 미팅 정리 Skill** — 녹취록을 붙여넣으면 짧은 시간에 회의록 완성 (사례 기준)
...
- 효과: 작성 시간이 크게 단축됨 (사례 기준). references/에 톤앤매너 가이드를 넣어서 다른 팀원이 써도 같은 퀄리티
...
```text
{작업명} Skill을 만들어줘.
Session 2에서 다룬 skill-template.md(.claude/skills/session2-skills/templates/skill-template.md)를 참고해서 .claude/skills/ 에 생성해줘.
