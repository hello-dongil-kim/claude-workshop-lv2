...
> ✅ **이렇게 되면 성공:** ① `/mcp`에 방금 추가한 서버가 목록에 보이고, ② 테스트 파일을 만들었을 때 설정한 Hook(시간 출력 등)이 자동으로 실행되면 성공.

---

## QUIZ

**문제 1 (회상형):** Notion에서 데이터를 가져와 파일로 저장할 때, 자동 백업도 하고 싶다면?
...
- B 오답: 역할이 뒤바뀌었다 — Hook은 연결 도구가 아니고, MCP는 백업 자동화 도구가 아니다.
- C 오답: MCP 서버 설정에 "자동 백업 옵션" 같은 건 없다 — 백업 자동화는 Hook의 몫이다.

**문제 2 (적용형):** 다음 상황에서 가장 적절한 것은? — `claude mcp add`로 Notion을 추가했는데 `/mcp` 목록에 안 보인다.

| 선택지 | 내용 |
|--------|------|
| A | 일단 Notion에 데이터를 요청해 보고 응답이 오는지로 판단한다 |
| B | "방금 추가한 MCP 서버가 안 보여. `claude mcp list`로 등록 상태를 확인하고 문제를 찾아줘"라고 Claude에게 시킨다 |
| C | settings.json을 직접 열어 한 줄씩 손으로 고친다 |

**정답:** B — 목록에 안 보일 때는 명령어 오타·URL 오류가 흔하다. 등록 상태부터 확인하고, 디버깅도 Claude에게 맡기는 것이 빠르다.
- A 오답: 목록에 없는 서버에 요청하면 실패만 반복된다 — 등록 여부부터 확인해야 한다.
- C 오답: JSON을 직접 손볼 필요가 없다 — Claude에게 확인·수정을 시키면 된다.
